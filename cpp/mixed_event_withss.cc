...
int main(int argc, char *argv[])
{
		...
		for (Long64_t ievent = 0; ievent < nevent; ievent++) {
			...
			for (ULong64_t icluster = 0; icluster < ncluster_max; icluster++) {
				...
				for (ULong64_t ijet = 0; ijet < njet; ijet++) {
                    ...
                    hCorrBR->Fill(corr, purity_weight - 1);
                    hCorr1ptBR->Fill(corr, (purity_weight - 1) / jetpt);
                    hCorrBR_jetmultcorrected->Fill(corr, (purity_weight - 1) * jetmultcorrection);
                    hCorr1ptBR_jetmultcorrected->Fill(corr, (purity_weight - 1) / jetpt * jetmultcorrection);
				} // end jet loop
			} // end cluster loop
	...
	std::string extendedname = basename.replace(basename.find(".root"), 5, "_withss_" + mixlabel + ".root");
	if (argc > 3) {
